...
class Cx extends Taglib
{
    ...
    public function tagLoad(array $tag, string $content): string
    {
		...
		$lineBreak = PHP_EOL;
        $this->tpl->getConfig('strip_space') && ($lineBreak = '');
        foreach ($array as $val) {
            $type = strtolower(substr(strrchr($val, '.'), 1));
            switch ($type) {
                case 'js':
                    $parseStr .= "<script type=\"text/javascript\" src=\"{$val}\"></script>{$lineBreak}";
                    break;
                case 'css':
                    $parseStr .= "<link rel=\"stylesheet\" type=\"text/css\" href=\"{$val}\"/>{$lineBreak}";
                    break;
				case 'ico':
                    $parseStr .= "<link rel=\"shortcut icon\" href=\"{$val}\"/>{$lineBreak}";
                    break;
                case 'php':
