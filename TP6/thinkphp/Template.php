...
class Template
{
    ...
    public function fetch(string $template, array $vars = []) : void
    {
        ...
        if ($this->hasCache($this->config['cache_id'])) {
            // 读取渲染缓存
            $this->send($this->readCache($this->config['cache_id']));
            return;
        }
		...
		if (is_array($template)) {
            ...
            if ($template) {
                ...
                if (!$this->checkCache($cacheFile)) {
                    // 缓存无效 重新模板编译
                    $content = $this->storage->getFile($template);
                ...
                // 缓存页面输出
                $this->writeCache($this->config['cache_id'], $content, $this->config['cache_time']);
     ...
     * @return boolean
     */
    public function hasCache(string $cacheId) : bool
    {
     ...
     * @return void
     */
    public function writeCache(string $cacheId, string $content, int $cache_time) : void
    {
        if ($cacheId && $this->cache && $this->config['display_cache']) {
            // 缓存页面输出
            $this->cache->set($cacheId, $content, $cache_time);
        }
    }
     ...
     * @return mixed
     */
    public function readCache(string $cacheId)
    {
        if ($cacheId && $this->cache && $this->config['display_cache']) {
    ...
    public function display(string $content, array $vars = []) : void
    {
        ...
        // 缓存页面输出
        $this->writeCache($this->config['cache_id'], $contents, $this->config['cache_time']);
     ...
     * @return $this
     */
    public function layout($name, string $replace = '') : self
    {
        if (false === $name) {
            // 关闭布局
    ...
    {
        if (!$this->config['tpl_cache'] || !is_file($cacheFile) || !$handle = @fopen($cacheFile, "rb")) {
            return false;
        }
    ...
    private function compiler(string &$content, string $cacheFile) : void
    {
        ...
        if ($this->config['layout_on']) {
            if (false !== strpos($content, '{__NOLAYOUT__}')) {
				...
				if (is_array($layoutFile)) {
                    ...
                    if ($layoutFile) {
                        // 替换布局的主体内容
                        $content = str_replace($this->config['layout_item'], $content, $this->storage->getFile($layoutFile));
                    }
                }
            }
        } else {
        ...
        // 添加安全代码及模板引用记录
        $content = '<?php if (!defined(\'DEBUG_LEVEL\')) exit(); /*' . serialize($this->includeFile) . '*/ ?>' . PHP_EOL . $content;
        // 编译存储
    ...
    private function parsePhp(string &$content) : void
    {
		...
		// 短标签的情况要将<?标签用echo方式输出 否则无法正常输出xml标识
        $content = preg_replace('/(<\\?(?!php|=|$))/i', '<?php echo \'\\1\'; ?>' . PHP_EOL, $content);
		
        // 过滤eval函数，防止被注入执行任意代码 by 仰融
        $view_replace_str = isset($this->config['view_replace_str']) ? $this->config['view_replace_str'] : '';
        ...
        if ($this->config['tpl_deny_php'] && false !== strpos($content, '<?php')) {
            ...
            // 运营模式下继续模板渲染 by 仰融
            $this->send($this->app->lang->get('not allow php tag'));
        }
    }
            ...
            if (!$this->config['layout_on'] || $this->config['layout_name'] != $array['name']) {
				...
				if (is_array($layoutFile)) {
                    ...
                    if ($layoutFile) {
                        ...
                        // 替换布局的主体内容
                        $content = str_replace($replace, $content, $this->storage->getFile($layoutFile));
                    }
                }
            }
        } else {
            ...
            if (preg_match_all($regex, $template, $matches, PREG_SET_ORDER)) {
                foreach ($matches as $match) {
                    ...
                    if (preg_match('/^([^\\/]+)\\.' . $this->config['view_suffix'] . '$/i', $file) === 1) {
                        // 支持模板引入的写法： {include file="header.htm" /} 或者 {include file="header" /}  或者 {include file="users/header.htm" /} 或者 {include file="users/header" /}
                        $file = str_replace('.' . $this->config['view_suffix'], '', $file);
                    } else {
                        if (stristr($file, '.' . $this->config['view_suffix']) && (stristr($file, '/') || stristr($file, '\\'))) {
    ...
    public function parseTagLib(string $tagLib, string &$content, bool $hide = false) : void
    {
        ...
        if (class_exists($className)) {
            $tLib = new $className($this->app, $this);
    ...
    public function parseVarFunction(string &$varStr, bool $autoescape = true) : string
    {
		if (!$autoescape && false === strpos($varStr, '|')) {
            ...
            // 修复函数不存在时报错的情况
            foreach ($this->config['default_filter'] as $key => $filter_function) {
                if ($filter_function && !function_exists($filter_function)) {
                    continue;
                }
                $default_filter[$key] = $filter_function;
            }
            $default_filter = implode('|', $default_filter);
            $varStr .= '|' . $default_filter;
        }
    ...
    public function parseRequestVar(array $vars) : string
    {
        ...
        switch ($type) {
            case 'SERVER':
                ...
                $parseStr = 'app(\'request\')->' . $method . '(\'' . $param . '\')';
                break;
            case 'GET':
                //$parseStr = '$_GET[\'' . $param . '\']';
				$parseStr = 'app(\'request\')->' . $method . '(\'' . $param . '\')';
    ...
    private function parseTemplateName(string $templateName) : string
    {
        ...
        foreach ($array as $templateName) {
			...
			if (is_array($template)) {
                ...
                if ($template) {
                    // 获取模板文件内容
                    $parseStr .= $this->storage->getFile($template);
                }
            }
        }
    ...
    private function parseTemplateFile(string $template)
    {
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
			...
			$templatebase = $template . '.' . ltrim($this->config['view_suffix'], '.');
            $template = $this->config['view_path'] . $templatebase;
        }
		// by 仰融
		if (!is_file($template)) {
            ...
            $templateoverbase = ltrim($template, $this->config['view_depr']);
           
            $template = $this->config['view_path'] . $templateoverbase;
            if (!is_file($template)) {
                $template = str_replace(['/', '\\'], $this->config['view_depr'], $this->app->getRootPath() . 'view' . $this->config['view_depr'] . $templateoverbase);
            }
        }
        ...
        // 引入模板不存在不影响页面的渲染 by 仰融
        return ['code' => 0, 'msg' => $this->app->lang->get('template not exists') . ': ' . $template];
    }
