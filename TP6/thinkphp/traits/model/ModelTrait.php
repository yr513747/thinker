...
trait ModelTrait
{
    public static function get($where)
    {
        if (!is_array($where)) {
            ...
            return self::where($where)->getOne();
        }
    }
    ...
    public static function getModelTime($where, $model = null, $prefix = 'add_time', $data = 'data', $field = ' - ')
    {
        ...
        switch ($where[$data]) {
            ...
            case 'quarter':
                ...
                break;
            case 'lately7':
                $model = $model->where($prefix, 'between', [strtotime("-7 day"), getTime()]);
                break;
            case 'lately30':
                $model = $model->where($prefix, 'between', [strtotime("-30 day"), getTime()]);
                break;
            default:
