/**************************************************
 * YROnlinePlayer v2.41
 * 播放器主功能模块
 * 编写：yangrong(https://popcom.com)
 * 时间：2018-3-13
 *************************************************/
// 播放器功能配置
var mkPlayer = {
    api: YROnlinePlayer, // api地址
...
if (mkPlayer.autoplay && mkPlayer.qklg){
mkPlayer.autoplay = false;
};
...
function audioPlay() {
    ...
    $(".btn-play").addClass("fa-pause");        // 恢复暂停
    ...
    var msg = " 正在播放: " + music.name + " - " + music.artist+"            ";  // 改变标题
...
function titleFlash(msg) {
   ...
   // rem.titflash = setInterval(function(){tit()}, 6000);
	rem.titflash = tit();
...
function audioPause() {
    ...
    $(".btn-play").removeClass("fa-pause");     // 取消暂停
...
//console.info('欢迎使用 YROnlinePlayer!\n当前版本：'+yrPlayer.version+' \n作者：yangrong(https://popcom.com)\n歌曲来源于各大音乐平台\n');
//layer.msg('欢迎使用 YROnlinePlayer!\n当前版本：'+yrPlayer.version+' \n作者：yangrong(https://popcom.com)\n歌曲来源于各大音乐平台\n');
...
function vBcallback(newVal) {
    ...
    if($(".btn-quiet").is('.fa-bell-slash')) {
        $(".btn-quiet").removeClass("fa-bell-slash");     // 取消静音
    }
    
    if(newVal === 0) $(".btn-quiet").addClass("fa-bell-slash");
...
var initProgress = function(){  
    ...
    if(tmp_vol == 0) $(".btn-quiet").addClass("fa-bell-slash"); // 添加静音样式
