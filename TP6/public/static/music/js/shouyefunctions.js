...
$(function(){
    ...
    $(".btn-quiet").click(function(){
        ...
        if($(this).is('.fa-bell-slash')) {
            ...
            $(this).removeClass("fa-bell-slash");     // 取消静音
        } else {
            oldVol = volume_bar.percent;
            $(this).addClass("fa-bell-slash");        // 开启静音
    ...
    // 图片加载失败处理
    $('img').error(function(){
        $(this).attr('src', '/public/music/images/player_cover.png');
    });
...
function download(music) {
    ...
    downLoadByUrl(music.url, music.name + ' - ' + music.artist);
}
...
function changeCover(music) {
    ...
    if(img == "err") {
        img = "/public/music/images/player_cover.png";
    } else {
...
function addSheet(no, name, cover) {
    if(!cover) cover = "/public/music/images/player_cover.png";
    if(!name) name = "读取中...";
...
function clearDislist() {
    ...
    if(rem.dislist == 1) {  // 正在播放列表
        ...
        $(".sheet-item[data-no='1'] .sheet-cover").attr('src', '/public/music/images/player_cover.png');    // 恢复正在播放的封面
...
*https://www.cnblogs.com/roseAT/p/11074765.html
*/
