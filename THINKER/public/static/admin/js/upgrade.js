...
function btn_upgrade(obj, type)
{
    ...
    if (undefined == filelist || !filelist) {
        ...
        var alert1 = layer.alert("请清除后台缓存以及Ctrl+F5强制刷新页面，再尝试升级！", {icon: 7, title:false}, function(){
            layer.close(alert1);
            var url = thinker_basefile + "?"+var_app+"="+app_name+"&"+var_controller+"=System&"+var_action+"=clearCache";
                        ...
                        if (true == finish) {
                            ...
                            var full = parent.layer.alert(title, {
                                    ...
                                    if (version < max_version) { // 当前升级之后的版本还不是官方最新版本，将继续连续更新
                                        ...
                                        var url = thinker_basefile + "?"+var_app+"="+app_name+"&"+var_controller+"=Tools&"+var_action+"=index";
                                        var iframe = $(obj).data('iframe');
                                        if ('parent' == iframe) {
                                            top.location.href = thinker_basefile;
                                            // workspace.window.location.href = url;
                                        } else {
                                            top.location.href = thinker_basefile;
                                            // window.location.href = url;
...
function export_data(){
    ...
    setTimeout(function(){
        var url = thinker_basefile + "?"+var_app+"="+app_name+"&"+var_controller+"=Tools&"+var_action+"=export&_ajax=1";
        $.ajax({
            url: url,
            data: {tables:'all'},
            type:'post',
