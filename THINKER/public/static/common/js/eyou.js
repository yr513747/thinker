...
jQuery(function($){
     ...
     */
    $('.thinker-edit').mouseenter(function(e){ // 鼠标移入选中状态，只针对该绑定元素
        e.stopPropagation();
        var that = this;
        thinker_mouseenter(that);
    })
    .mouseleave(function(e){ // 鼠标移出消除选中状态，只针对该绑定元素
        e.stopPropagation();
        var that = this;
        thinker_mouseleave(that);
    });

    // 鼠标移入选中状态，只针对该绑定元素
    function thinker_mouseenter(that)
    {
        $(that).addClass('uiset');
        ...
        $(that).find('b.ui_icon').on("click", function(e){
            ...
            if (e_type == 'text') {
                ...
                thinker_arclist(that);
            } else if (e_type == 'channel') {
                thinker_channel(that);
    ...
    // 鼠标移出消除选中状态，只针对该绑定元素
    function thinker_mouseleave(that)
    {
 ...
 * 获取修改之前的内容
 */
function thinker_getOldHtml()
{
    return oldhtml;
}

function thinker_showErrorMsg(msg){
    layer.msg(msg, {icon: 5,time: 2000});
}

function thinker_showSuccessMsg(msg){
 ...
 * 封装的加载层
 */
function thinker_layer_loading(msg){
 ...
 * 封装的加载层，用于iframe
 */
function thinker_iframe_layer_loading(msg){
