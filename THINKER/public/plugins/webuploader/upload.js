$(function() {
			...
			if ( file.getStatus() === 'invalid' ) {
				...
				uploader.makeThumb( file, function( error, src ) {
					...
					if( isSupportBase64 ) {
						...
						$.ajax(thinker_basefile+"?"+var_app+"="+app_name+"&"+var_controller+"=Uploadify&"+var_action+"=preview", {
							method: 'POST',
							data: src,
							dataType:'json'
	...
	window.Manager = {
		...
		del:function(obj){
			if(!confirm('确定删除吗?')) return false;
		    $.post(thinker_basefile+"?"+var_app+"="+app_name+"&"+var_controller+"=Uploadify&"+var_action+"=delupload",{action:"del", filename:$(obj).attr('data-img')},function(res){
					if(res == 1){
