...
function MakeOneTag(&$dtp, &$refObj, $parfield='Y')
{
    ...
    foreach($dtp->CTags as $tagid=>$ctag)
    {
        ...
        if(in_array($tagname,$alltags))
        {
            ...
            {
                if(DEBUG_LEVEL) echo 'DedeCMS Error:Tag disabled:"'.$tagname.'" <a href="http://help.thinker.com/install-use/apply/2013/0711/2324.html" target="_blank">more...</a>!';
                continue;
            }
            if (DEBUG_LEVEL==TRUE) {
                $ttt1 = ExecTime();
...
function GetCacheBlock($cacheid)
{
    ...
    if(!file_exists($cachefile) || filesize($cachefile)==0 || 
      $cfg_puccache_time==0 || getTime() - filemtime($cachefile) > $cfg_puccache_time)
    {
        return '';
    }
