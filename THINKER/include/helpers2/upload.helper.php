...
if ( ! function_exists('AdminUpload'))
{
    function AdminUpload($uploadname, $ftype='image', $rnddd=0, $watermark=TRUE, $filetype='' )
    {
        ...
        if($ftype=='image' || $ftype=='imagelit')
        {
            ...
            $filedir = $cfg_image_dir.'/'.MyDate($cfg_addon_savetype, getTime());
        }
        else if($ftype=='media')
        {
            $filetype = '3';
            if( !preg_match('/'.$cfg_mediatype.'/', $file_sname) ) return 0;
            $filedir = $cfg_other_medias.'/'.MyDate($cfg_addon_savetype, getTime());
        }
        else
        {
            $filetype = '4';
            ...
            $filedir = $cfg_soft_dir.'/'.MyDate($cfg_addon_savetype, getTime());
        }
        ...
        $filename = $cuserLogin->getUserID().'-'.dd2char(MyDate('ymdHis', getTime())).$rnddd;
        ...
        $inquery = "INSERT INTO `#@__uploads`(title,url,mediatype,width,height,playtime,filesize,uptime,mid)
            VALUES ('$title','$fileurl','$filetype','0','0','0','".filesize($cfg_basedir.$fileurl)."','".getTime()."','".$cuserLogin->getUserID()."'); ";
