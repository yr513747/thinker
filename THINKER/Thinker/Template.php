...
class Template
{
    ...
    protected $config = [
        ...
        'taglib_build_in'    => 'thinker', // 内置标签库名称(标签使用不必指定标签库名称),以逗号分隔 注意解析顺序
        'taglib_pre_load'    => '', // 需要额外加载的标签库(须指定标签库名称)，多个以逗号分隔
        'display_cache'      => false, // 模板渲染缓存
        'cache_id'           => '', // 模板缓存ID
        'tpl_replace_string' => [],
        'tpl_var_identify'   => 'array', // .语法变量识别，array|object|'', 为空时自动识别
        'default_filter'     => ['strip_sql', 'htmlspecialchars', 'htmlentities', 'nl2br', 'htmlspecialchars_decode'], // 默认过滤方法 用于普通标签输出
    ];
    ...
    public function __construct(array $config = [])
    {
        ...
        $class = false !== strpos($type, '\\') ? $type : '\\Thinker\\template\\driver\\' . ucwords($type);
    ...
    public function fetch(string $template, array $vars = []): void
    {
		...
		if (is_array($template)) { // 引入模板错误的友好提示 by 仰融
    ...
    private function compiler(string &$content, string $cacheFile): void
    {
        ...
        if ($this->config['layout_on']) {
            if (false !== strpos($content, '{__NOLAYOUT__}')) {
				...
				if (is_array($layoutFile)) { // 引入模板的错误友好提示 by 仰融
        ...
        // 添加安全代码及模板引用记录
		$content = '<?php if (!defined(\'DEBUG_LEVEL\')) exit(); /*' . serialize($this->includeFile) . '*/ ?>' . "\n" . $content;
        // 编译存储
    ...
    private function parsePhp(string &$content): void
    {
        ...
        $view_replace_str = $this->config['view_replace_str'];
        if (isset($view_replace_str['__EVAL__'])) {
            if (stristr($content, '{php}')) { // 针对{php}标签语法处理
        ...
        if ($this->config['tpl_deny_php'] && false !== strpos($content, '<?php')) {
            if (App::isDebug()) { // 调试模式下中断模板渲染 by 仰融
                throw new Exception('not allow php tag', 11600);
            } else { // 运营模式下继续模板渲染 by 仰融
                echo(Lang::get('not allow php tag'));
            }
        }
    }
    ...
    private function parseLayout(string &$content): void
    {
        ...
        if (preg_match($this->getRegex('layout'), $content, $matches)) {
            ...
            if (!$this->config['layout_on'] || $this->config['layout_name'] != $array['name']) {
				...
				if (is_array($layoutFile)) { // 引入模板错误的友好提示 by 仰融
                    $content = !empty($layoutFile['msg']) ? $layoutFile['msg'] : $content;
                } else if ($layoutFile) {
    ...
    public function parseTagLib(string $tagLib, string &$content, bool $hide = false): void
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\Thinker\\template\\taglib\\' . ucwords($tagLib);
        }
    ...
    public function parseVarFunction(string &$varStr, bool $autoescape = true): string
    {
        if (!$autoescape && false === strpos($varStr, '|')) {
			...
			//by仰融
			$default_filter = implode('|', $this->config['default_filter']);
    ...
    public function parseThinkVar(array $vars): string
    {
        ...
        switch ($type) {
            case 'CONST':
                ...
                break;
            case 'NOW':
                $parseStr = "date('Y-m-d g:i a',getTime())";
                break;
            case 'LDELIM':
    ...
    private function parseTemplateFile(string $template)
    {
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
			...
			$template_sub = str_replace(['/', ':'], $this->config['view_depr'], $template);
			
			$template_sub = substr($template_sub, 1);
			
			$template_sub = ltrim($template_sub, $this->config['view_depr']);
			
            $template = $this->config['view_path'] . $template_sub;	
			if (!is_file($template)) {
                $template = root_path('view') . $template_sub;
            }	
        }
