...
use think\App;
use think\Response;
use Thinker\View as ViewTemplate;
...
trait Jump
{
    ...
    protected function success($msg = '', string $url = null, $data = '', int $wait = 1, array $header = [], string $target = '_self')
    {
        ...
        if ('html' == strtolower($type)) {
            ...
            $ViewTemplate = ViewTemplate::instance($template);
            $view = $this->app->config->get('app.jump_success_tmpl');
            $result = $ViewTemplate->fetch($view, $result);
        }
    ...
    protected function error($msg = '', string $url = null, $data = '', int $wait = 5, array $header = [], string $target = '_self')
    {
        ...
        if ('html' == strtolower($type)) {
            $template = $this->app->config->get('template');
            $ViewTemplate = ViewTemplate::instance($template);
            $view = $this->app->config->get('app.jump_error_tmpl');
            $result = $ViewTemplate->fetch($view, $result);
        }
        $response = Response::create($result, $type)->header($header);
        throw new HttpResponseException($response);
     ...
     * @throws HttpResponseException
     */
    protected function img(string $data = '', int $code = 200, array $header = [], array $options = [], string $contentType = 'image/png')
    {
        $response = Response::create($data, 'image', $code)->header($header)->contentType($contentType)->options($options);
        throw new HttpResponseException($response);
    }
    /**
    ...
    {
        $type = $this->getResponseType();
        return $this->tpDie($data, $type, $code, $header, $options);
    }
    /**
     * 中断输出
     ...
     * @return void
     */
    protected function getResponse($data = '', string $type = '', int $code = 200, array $header = [], array $options = [])
    {
        $type = $type ? $type : $this->getResponseType();
        if (is_object($data)) {
     ...
     */
    protected function encode(string $string, string $key = '', int $expiry = 0)
    {
        return Mcrypt::encode($string, $key = '', $expiry = 0);
    }
    /**
     * 字符解密，一次一密,可定时解密有效
     * 
     ...
     */
    protected function decode(string $string, string $key = '')
    {
        return Mcrypt::decode($string, $key = '');
    }
}