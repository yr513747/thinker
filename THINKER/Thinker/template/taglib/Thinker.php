...
namespace Thinker\template\taglib;

use Thinker\template\TagLib;

class Thinker extends Taglib
{
    ...
    protected $tags = [
        ...
        // 'sql'        => ['attr' => 'sql,key,id,mod,cachetime,empty', 'close'=>1, 'level'=>3], 
        'weapp'      => ['attr' => 'type', 'close' => 0], // 网站应用插件
		'lang'       => ['attr' => 'name', 'close' => 0],
		...
		// 网站应用插件列表
        'weapplist'  => ['attr' => 'type,id,key,mod,empty,currentstyle'], 
    ...
    public function tagSql($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagSql = new \Thinker\template\taglib\thinker\TagSql;';
    ...
    public function tagLoad($tag)
    {
        ...
        $parseStr .= $startStr;
        $parseStr .= ' <? $tagLoad = new \Thinker\template\taglib\thinker\TagLoad;';
    ...
    /**
     * static 标签解析 {static file="/static/js/base.js" /}
     * 格式：{static file="/static/css/base.css" /}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagStatic($tag)
    {
        ...
        $parseStr .= ' $tagStatic = new \Thinker\template\taglib\thinker\TagStatic;';
     ...
     * {channel typeid='1' type='son' row='10' reid='0' empty='' name='' id='' key='' titlelen='' offset='' mod='' currentstyle='active'}
    ...
    public function tagChannel($tag, $content)
    {
        ...
        if ($name) { // 从模板中传入数据集
            ...
            $parseStr .= ' $tagChannel = new \Thinker\template\taglib\thinker\TagChannel;';
     ...
     * 格式：type:son表示下级栏目,self表示同级栏目,top顶级栏目
     * {channelartlist typeid='1' type='son' row='10'}
     *  <li><a href='{$field:typelink}'>{$field:typename}</a> </li> 
     * {/channelartlist}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagChannelartlist($tag, $content)
    {
        ...
        $parseStr .= ' $tagChannelartlist = new \Thinker\template\taglib\thinker\TagChannelartlist;';
     ...
     * 格式：
     * {arclist channelid='1' typeid='1' row='10' offset='0' titlelen='30' orderby ='aid desc' flag='' infolen='160' empty='' id='field' mod='' name=''}
     *  {$field.title}
     *  {$field.typeid}
     * {/arclist}
     * @access public
    ...
    public function tagArclist($tag, $content)
    {
        ...
        if ($name) { // 从模板中传入数据集
            ...
            $parseStr .= ' $tagArclist = new \Thinker\template\taglib\thinker\TagArclist;';
     ...
     * 格式：
     * {list channelid='1' typeid='1' row='10' titlelen='30' orderby ='aid desc' flag='' infolen='160' empty='' id='field' mod='' name=''}
     *  {$field.title}
     *  {$field.typeid}
     * {/list}
     * @access public
    ...
    public function tagList($tag, $content)
    {
        ...
        $parseStr .= ' $tagList = new \Thinker\template\taglib\thinker\TagList;';
     ...
     * pagelist 标签解析
     * 在模板中获取列表的分页
     * 格式： {pagelist listitem='info,index,end,pre,next,pageno' listsize='2'/}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagPagelist($tag)
    {
        ...
        $parseStr .= ' $tagPagelist = new \Thinker\template\taglib\thinker\TagPagelist;';
     ...
     * 格式： {arcpagelist tagid='' pagesize='2'} {/arcpagelist}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagArcpagelist($tag, $content)
    {
        ...
        $parseStr .= ' $tagArcpagelist = new \Thinker\template\taglib\thinker\TagArcpagelist;';
    ...
    public function tagPosition($tag)
    {
        ...
        $parseStr .= ' $tagPosition = new \Thinker\template\taglib\thinker\TagPosition;';
    ...
    public function tagSearchform($tag, $content)
    {
        ...
        $parseStr .= ' $tagSearchform = new \Thinker\template\taglib\thinker\TagSearchform;';
    ...
    public function tagType($tag, $content)
    {
        ...
        $parseStr .= ' $tagType = new \Thinker\template\taglib\thinker\TagType;';
    ...
    public function tagArcview($tag, $content)
    {
        ...
        $parseStr .= ' $tagArcview = new \Thinker\template\taglib\thinker\TagArcview;';
    ...
    public function tagTag($tag, $content)
    {
        ...
        $parseStr .= ' $tagTag = new \Thinker\template\taglib\thinker\TagTag;';
    ...
    public function tagFlink($tag, $content)
    {
        ...
        $parseStr .= ' $tagFlink = new \Thinker\template\taglib\thinker\TagFlink;';
    ...
    public function tagAd($tag, $content)
    {
        ...
        $parseStr .= ' $tagAd = new \Thinker\template\taglib\thinker\TagAd;';
    ...
    public function tagAdv($tag, $content)
    {
        ...
        $parseStr .= ' $tagAdv = new \Thinker\template\taglib\thinker\TagAdv;';
    ...
    public function tagPrenext($tag, $content)
    {
        ...
        if (1 == $style) {
            ...
            $parseStr = '<?php ';
            $parseStr .= ' $tagPrenext = new \Thinker\template\taglib\thinker\TagPrenext;';
    ...
    public function tagGuestbookform($tag, $content)
    {
        ...
        $parseStr .= ' $tagGuestbookform = new \Thinker\template\taglib\thinker\TagGuestbookform;';
     ...
     * 格式： {field name="typename|html_msubstr=###,0,2" /}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
    public function tagField($tag)
    {
        ...
        if (!empty($name)) {
            ...
            $parseStr .= ' $tagField = new \Thinker\template\taglib\thinker\TagField;';
    ...
    public function tagVolist($tag, $content)
    {
        ...
        /*用于下载模型的ajax下载文件*/
        $parseStr .= '<?php echo isset($'.$id.'["thinker_1563185380"])?$'.$id.'["thinker_1563185380"]:""; ?>';
        $parseStr .= '<?php echo (1 == $e && isset($'.$id.'["thinker_1563185376"]))?$'.$id.'["thinker_1563185376"]:""; ?>';
        /*end*/
    ...
    public function tagGlobal($tag)
    {
        ...
        $parseStr .= ' $tagGlobal = new \Thinker\template\taglib\thinker\TagGlobal;';
    ...
    public function tagArcclick($tag)
    {
        ...
        $parseStr .= ' $tagArcclick = new \Thinker\template\taglib\thinker\TagArcclick;';
    ...
    public function tagDowncount($tag)
    {
        ...
        $parseStr .= ' $tagDowncount = new \Thinker\template\taglib\thinker\TagDowncount;';
     ...
     * weapp标签解析
     * 安装网站应用插件时自动在页面上追加代码
     * 格式： {weapp type='default'}content{/weapp}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
    ...
    public function tagWeapp($tag, $content)
    {
        ...
        $parseStr = ' <?php ';
        $parseStr .= ' $tagWeapp = new \Thinker\template\taglib\thinker\TagWeapp;';
    ...
    public function tagLang($tag)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagLang = new \Thinker\template\taglib\thinker\TagLang;';
        $parseStr .= ' $__VALUE__ = $tagLang->getLang(\''.serialize($name).'\');';
        $parseStr .= ' echo $__VALUE__;';
        $parseStr .= ' ?>';
     ...
     * 格式：
     * {diyfield type="default" name="$thinker.field.imgs" id="field"}
    ...
    public function tagDiyfield($tag, $content)
    {
        ...
        $parseStr .= ' $tagDiyfield = new \Thinker\template\taglib\thinker\TagDiyfield;';
    ...
    public function tagAttribute($tag, $content)
    {
        ...
        $parseStr .= ' $tagAttribute = new \Thinker\template\taglib\thinker\TagAttribute;';
    ...
    public function tagAttr($tag)
    {
        ...
        $parseStr .= ' $tagAttr = new \Thinker\template\taglib\thinker\TagAttr;';
    ...
    public function tagUser($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagUser = new \Thinker\template\taglib\thinker\TagUser;';
     ...
     * weapplist标签解析
     * 安装网站应用插件时自动在页面上追加代码
     * 格式： {weapplist type='default'}content{/weapplist}
     * @access public
     * @param array $tag 标签属性
     * @param string $content 标签内容
     * @return string
    ...
    public function tagWeapplist($tag, $content)
    {
        ...
        $parseStr .= ' $tagWeapplist = new \Thinker\template\taglib\thinker\TagWeapplist;';
    ...
    public function tagUsermenu($tag, $content)
    {
        ...
        $parseStr .= ' $tagUsermenu = new \Thinker\template\taglib\thinker\TagUsermenu;';
    ...
    public function tagSppurchase($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagSppurchase = new \Thinker\template\taglib\thinker\TagSppurchase;';
    ...
    public function tagSpcart($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagSpcart = new \Thinker\template\taglib\thinker\TagSpcart;';
    ...
    public function tagSporder($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagSporder = new \Thinker\template\taglib\thinker\TagSporder;';
     ...
     * 格式：sort:排序方式 month，rand，week
     *       getall:获取类型 0 为当前内容页TAG标记，1为获取全部TAG标记
     * {spsubmitorder row='1' titlelen='20'}
     *  <li><a href='{$field:url}'>{$field:title}</a> </li> 
     * {/spsubmitorder}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagSpsubmitorder($tag, $content)
    {
        ...
        $parseStr = '<?php ';
        $parseStr .= ' $tagSpsubmitorder = new \Thinker\template\taglib\thinker\TagSpsubmitorder;';
    ...
    public function tagSporderlist($tag, $content)
    {
        ...
        $parseStr .= ' $tagSporderlist = new \Thinker\template\taglib\thinker\TagSporderlist;';
     ...
     * sppageorder 标签解析
     * 在模板中获取列表的分页
     * 格式： {sppageorder listitem='info,index,end,pre,next,pageno' listsize='2'/}
     * @access public
     * @param array $tag 标签属性
     * @return string
     */
     ...
     * 格式：sort:排序方式 month，rand，week
     *       getall:获取类型 0 为当前内容页TAG标记，1为获取全部TAG标记
     * {spordergoods row='1' titlelen='20'}
     *  <li><a href='{$field:url}'>{$field:title}</a> </li> 
     * {/spordergoods}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagSpaddress($tag, $content)
    {
        ...
        $parseStr .= ' $tagSpaddress = new \Thinker\template\taglib\thinker\TagSpaddress;';
    ...
    public function tagSpsearch($tag, $content)
    {
        ...
        $parseStr .= ' $tagSpsearch = new \Thinker\template\taglib\thinker\TagSpsearch;';
    ...
    public function tagScreening($tag, $content)
    {
        ...
        $parseStr .= ' $tagScreening = new \Thinker\template\taglib\thinker\TagScreening;';
    ...
    public function tagMemberlist($tag, $content)
    {
        ...
        $parseStr .= ' $tagMemberlist = new \Thinker\template\taglib\thinker\TagMemberlist;';
     ...
     * 格式：
     * {likearticle mytypeid='0' limit='0,10' titlelen='30' infolen='160' id='field'}
     * <a href="{$field:arcurl}">{$field:title}</a>
     * {/likearticle}
     * @access public
     * @param array $tag 标签属性
    ...
    public function tagLikearticle($tag, $content)
    {
        ...
        if ($name) { // 从模板中传入数据集
            ...
            $parseStr .= ' $tagLikearticle = new \Thinker\template\taglib\thinker\TagLikearticle;';
