...
class TagSppurchase extends Base
{
    ...
    public function getSppurchase($currentstyle = '')
    {
        ...
        $ShopConfig = UsersConfigModel::getUsersConfigData('shop');
        $web_users_switch = ConfigModel::tpCache('web.web_users_switch');
        if (empty($aid) || empty($ShopConfig['shop_open']) || empty($web_users_switch)) {
            return false;
        }
        $value = ConfigModel::tpCache('web.web_is_authortoken');
        ...
        $archivesInfo = Db::name('archives')->where($Where)->field('channel,users_price,old_price,stock_count,stock_show')->getOne();
        ...
        if (!isset($ShopConfig['shop_open_spec']) || empty($ShopConfig['shop_open_spec'])) {
            ...
            $product_spec_data = Db::name('product_spec_data')->where($SpecWhere)->order($order)->getArray();
            if (!empty($product_spec_data)) {
                ...
                $product_spec_value = Db::name('product_spec_value')->where($SpecWhere)->order('spec_price asc')->getArray();
        ...
        $result['hidden'] = <<<EOF
<input type="hidden" id="thinker_stock_1565602291" value="{$archivesInfo['stock_count']}">
<input type="hidden" id="SelectValueIds" value="{$SpecValueIds}">
<script type="text/javascript">
    var fe912b5dac71082e12c1827a3107f9b = {$data_json};
</script>
<link rel="stylesheet" type="text/css" href="{$this->root_dir}/static/common/css/shopcart.css?v={$version}">
<script type="text/javascript" src="{$this->root_dir}/static/common/js/tag_sppurchase.js?v={$version}"></script>
EOF;
        return $result;
    }
}