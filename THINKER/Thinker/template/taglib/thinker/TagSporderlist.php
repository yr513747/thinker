...
class TagSporderlist extends Base
{
    ...
    public function getSporderlist($pagesize = '10')
    {
        // 基础查询条件
        $OrderWhere = array();
        $OrderWhere[] = array('users_id', '=', $this->users_id);
        // 应用搜索条件
        $keywords = input('param.keywords/s');
        if (!empty($keywords)) {
            $OrderWhere[] = array('order_code', 'LIKE', "%{$keywords}%");
        }
        // 订单状态搜索
        ...
        if (!empty($select_status)) {
            ...
            $OrderWhere[] = array('order_status', '=', $select_status);
        }
        // 分页查询逻辑
        ...
        if (empty($result['list']) && !empty($keywords)) {
            $Data = M('Shop')->QueryOrderList($pagesize, $this->users_id, $keywords, $query_get);
            $result['list'] = $Data['list'];
            $result['pages'] = $Data['pages'];
        }
        // 规格值ID预处理
        $SpecValueArray = Db::name('product_spec_value')->field('aid,spec_value_id')->getArray();
        ...
        if (!empty($result['list'])) {
            ...
            foreach ($result['list'] as $key => $value) {
                $DetailsWhere[] = array('users_id', '=', $value['users_id']);
                $DetailsWhere[] = array('order_id', '=', $value['order_id']);
                // 查询订单明细表数据
                $result['list'][$key]['details'] = Db::name('shop_order_details')->order('product_price desc, product_name desc')->where($DetailsWhere)->getArray();
                ...
                foreach ($result['list'][$key]['details'] as $kk => $vv) {
                    ...
                    // 图片处理
                    $result['list'][$key]['details'][$kk]['litpic'] = handle_subdir(get_default_pic($vv['litpic']));
                }
            ...
            if (!empty($OrderIds)) {
                ...
                Db::name('shop_order')->where('order_id', 'IN', implode(',', $OrderIds))->update($UpData);
                // 追加订单操作记录
...
<script type="text/javascript" src="{$this->root_dir}/static/common/js/tag_sporderlist.js?v={$version}"></script>
EOF;
            return $result;
        } else {
            return false;
