...
abstract class Dispatch
{
    ...
    public function run(): Response
    {
		...
		if (is_array($this->dispatch)) {
            $this->app->log('[ DISPATCH_DEBUGINFO_dispatch ] ' . var_export($this->dispatch, true), 'info');
            $this->app->log('[ DISPATCH_DEBUGINFO_param ] ' . var_export($this->request->param(), true), 'info');
            $this->app->log('[ DISPATCH_DEBUGINFO_header ] ' . var_export($this->request->header(), true), 'info');
            $this->app->log('[ DISPATCH_DEBUGINFO_rule ] ' . var_export($this->rule->__debugInfo(), true), 'info');
        }

        $data = $this->exec();
		
		// index.php入口禁止访问的应用列表
        if (false !== strpos($this->request->baseFile(), 'index.php') && in_array($this->request->app(), $this->app->config->get('app.deny_multi_app_list', []))) {
            $beFile = $this->request->app() . '.php';
            if (file_exists($beFile)) {
                $baseFile = str_replace('index.php', $beFile, $this->request->baseFile());
                $data = Response::create($baseFile, 'redirect')->code(301);
            } else {
                $data = $this->app->setErrorPage($this->request, $this->app->config->get('app', []));
            }
        }
    ...
    protected function autoResponse($data): Response
    {
        if ($data instanceof Response) {
            ...
            $status = '' === $content && $this->request->isJson() || $this->request->isAjax() ? 204 : 200;
