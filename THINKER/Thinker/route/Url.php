...
class Url
{
    ...
    public function __construct(Route $route, App $app, string $url = '', array $vars = [])
    {
        ...
        $this->url   = $url;
        $this->vars  = $vars;
		/*$root_dir = isset($_M['root_dir']) ? $_M['root_dir'] : request()->rootUrl();
		$var_depr = config('route.pathinfo_depr', '/');
		$root_public = defined('IS_ROOT_ACCESS') ? $var_depr.$root_dir.'public'.$var_depr : $var_depr.$root_dir;
		$this->root($root_public.'?s=');*/
    }
    ...
    protected function getRuleUrl(array $rule, array &$vars = [], $allowDomain = ''): array
    {
        ...
        foreach ($rule as $item) {
			...
			/*同个模块、控制器、操作名对应多个路由规则，进行优先级别匹配 by 仰融*/
            $unequal = 0;
