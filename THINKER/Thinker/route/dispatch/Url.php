...
namespace Thinker\route\dispatch;
...
class Url extends Controller
{
    ...
    protected function parseUrl(string $url): array
    {
        ...
        $app = $multi_app ? array_shift($path) : null;
    ...
    {
        [$app, $controller, $action] = $route;
		if (empty($controller) || empty($action)) {
            return false;
        }

        // 检查地址是否被定义过路由
        $name = strtolower($app . '/' . Str::studly($controller) . '/' . $action);

        $name2 = '';

        if (empty($app) || $app == $bind) {
            $name2 = strtolower(Str::studly($controller) . '/' . $action);
        }
