...
$(function(){
    ...
    // 图片加载失败处理
    $('img').error(function(){
        $(this).attr('src', '/static/music/images/player_cover.png');
    });
...
function changeCover(music) {
    ...
    if(img == "err") {
        img = "/static/music/images/player_cover.png";
    } else {
...
function addSheet(no, name, cover) {
    if(!cover) cover = "/static/music/images/player_cover.png";
    if(!name) name = "读取中...";
...
function clearDislist() {
    ...
    if(rem.dislist == 1) {  // 正在播放列表
        ...
        $(".sheet-item[data-no='1'] .sheet-cover").attr('src', '/static/music/images/player_cover.png');    // 恢复正在播放的封面
