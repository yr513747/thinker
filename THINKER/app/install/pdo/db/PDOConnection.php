...
namespace app\install\pdo\db;

use Closure;
use PDO;
use PDOStatement;
use app\install\pdo\db\exception\BindParamException;
use app\install\pdo\db\exception\DataNotFoundException;
use app\install\pdo\db\exception\PDOException;
...
abstract class PDOConnection extends Connection implements ConnectionInterface
{
    ...
    {
        return $this->getConfig('builder') ?: '\\app\\install\\pdo\\db\\builder\\' . ucfirst($this->getConfig('type'));
    }
     ...
     * @return \Generator
     */
    public function getCursor(BaseQuery $query, string $sql, array $bind = [], $condition = null)
    {
    ...
    public function cursor(BaseQuery $query)
    {
        ...
        // 执行查询操作
        return $this->getCursor($query, $sql, $query->getBind(), $condition);
    }
