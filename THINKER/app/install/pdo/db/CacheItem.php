...
use app\install\pdo\db\exception\InvalidArgumentException;
...
class CacheItem
{
    ...
    public function getExpire()
    {
        ...
        return $this->expire ? $this->expire - getTime() : null;
    }
    ...
    public function expiresAfter($timeInterval)
    {
        if ($timeInterval instanceof DateInterval) {
            $this->expire = (int) DateTime::createFromFormat('U', (string) getTime())->add($timeInterval)->format('U');
        } elseif (is_numeric($timeInterval)) {
            $this->expire = $timeInterval + getTime();
        } else {
            throw new InvalidArgumentException('not support datetime');
        }
