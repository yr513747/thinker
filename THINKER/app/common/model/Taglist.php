...
class Taglist extends Model
{
     ...
     * @return    void
     */
    public function savetags($aid = 0, $typeid = 0, $tag = '', $opt = 'add')
    {
        if ($opt == 'add') {
            $tag = str_replace('，', ',', $tag);
            $tags = explode(',', $tag);
            foreach($tags as $tag)
            {
                ...
                $this->InsertOneTag($tag, $aid, $typeid);
            }
        } else if ($opt == 'edit') {
            $this->UpdateOneTag($aid, $typeid, $tag);
        }
    }
     ...
     * @return    void
     */
    private function InsertOneTag($tag, $aid, $typeid = 0)
    {
        $tag = trim($tag);
        if(empty($tag))
        {
     ...
     * @return    string
     */
    private function UpdateOneTag($aid, $typeid, $tags='')
    {
        if(!empty($tags))
        {
