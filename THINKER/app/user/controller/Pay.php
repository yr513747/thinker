...
class Pay extends Base
{
    ...
    public function pay_recharge_detail()
    {
        ...
        $auth_code = tpCache('system.system_auth_code');
        if(empty($querystr) || empty($hash) || md5("payment".$querystr.$auth_code) != $hash) {
            $this->error('无效订单！');
        }
    ...
    public function get_alipay_order()
    {
        if (isAjaxPost(false)) {
            ...
            $Result = $this->shop_order_db->where($where)->field('order_id, pay_name')->find();
            if (empty($Result)) $this->error('订单信息错误，请尝试刷新');
            // if ('alipay' !== $Result['pay_name']) {
            //     $UpData = [
            //         'pay_name' => 'alipay',
            //         'wechat_pay_type' => ''
            //     ];
            //     $this->shop_order_db->where($where)->update($UpData);
            // }
