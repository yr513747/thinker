...
use think\facade\Db;
use app\user\service\Shop as ShopService;
use app\common\model\UsersConfig as UsersConfigModel;
class Shop extends BaseController
{
	/**
     * 商城服务层 
     * @var app\user\service\Shop
     */
    ...
    protected function initialize() {
        ...
        if (!empty($redirect_url)) {
            ...
            return $this->error($msg, $redirect_url);
    ...
    public function shopCentre()
    {
        ...
        // 查询订单是否为空
        $result['data'] = $this->shop_service->GetOrderIsEmpty($this->users_id,$keywords,$select_status);
        // 是否移动端，1表示手机端，0表示PC端
        ...
        $this->assign('thinker',$thinker);
        return $this->fetch('users/shop_centre');
    }

    // 订单数据详情
    public function shopOrderDetails()
    {
        if (isGet()) {
            // 数据由标签调取生成
    ...
    // 订单提交
    public function shopUnderOrder($error = 'true')
    {
        if (empty($error)) {
            $this->error('您的购物车还没有商品！');
        }
        // 获取当前页面URL，存入session，若操作添加地址后返回当前页面
        session($this->users_id.'_ThinkerShopOrderUrl', $this->request->url(true));
        // 数据由标签调取生成
        return $this->fetch('users/shop_under_order');
    }

    // 购物车库存检测
    public function cartStockDetection()
    {
        if (isAjaxPost(false)) {
            // 购物车查询条件
            $CartWhere = [
            ...
            $list = Db::name('shop_cart')->field('a.product_num, b.stock_count, c.spec_value_id, c.spec_stock')
                ->alias('a')
    ...
    public function shopBuyNow()
    {
        if (isAjaxPost(false)) {
            ...
            $count = Db::name('archives')->where($archives_where)->count();
            // 跳转下单页
            if (!empty($count)) {
                // 对ID和订单号加密，拼装url路径
                $querydata = [
    ...
    public function shopAddCart()
    {
        if (isAjaxPost(false)) {
            ...
            if (!empty($count)) {
                ...
                $cartInfo = Db::name('shop_cart')->field('product_num')->where($cart_where)->find();
                if (!empty($cartInfo)) {
                    // 购物车内已有相同产品，进行数量更新。
                    $data['product_num'] = $param['num'] + intval($cartInfo['product_num']); //与购物车数量进行叠加
                    $data['update_time'] = getTime();
                    $cart_id = Db::name('shop_cart')->where($cart_where)->update($data);
                }else{
                    // 购物车内还未有相同产品，进行添加。
                    ...
                    $cart_id = Db::name('shop_cart')->add($data);
                }
                if (!empty($cart_id)) {
    ...
    // 统一修改购物车数量
    // symbol 加或减数量或直接修改数量
    public function cartUnifiedAlgorithm(){
        if (isAjaxPost(false)) {
            $aid    = input('post.aid');
            ...
            $archives_count = Db::name('archives')->where($archives_where)->count();
            if (!empty($archives_count)) {
                ...
                $product_num = Db::name('shop_cart')->where($cart_where)->getField('product_num');
                // 处理购物车产品数量
    ...
    public function cartDel()
    {
        if (isAjaxPost(false)) {
            ...
            if (!empty($cart_id)) {
                ...
                // 删除数据
                $return = Db::name('shop_cart')->where($cart_where)->delete();
            }
            if (!empty($return)) {
                /*计算购物车总数总额*/
                $CaerWhere = [
    ...
    public function cartChecked()
    {
        if (isAjaxPost(false)) {
            ...
            // 更新数据
            $return = Db::name('shop_cart')->where($cart_where)->update($data);
            if (!empty($return)) {
                $this->success('操作成功！');
            }else{
                $this->error('操作失败！');
    ...
    public function shopWechatPaySelect()
    {
        $ReturnOrderData = session($this->users_id.'_ReturnOrderData');
        if (empty($ReturnOrderData)) {
            $url = session($this->users_id.'_ThinkerShopOrderUrl');
    ...
    public function shopPaymentPage()
    {
        if (isPost()) {
            // 提交的订单信息判断
            $post = input('post.');
            ...
            if (!empty($aid)) {
                ...
                $field = 'a.aid, a.aid as product_id, a.title, a.litpic, a.users_price, a.stock_count, a.prom_type, b.spec_price, b.spec_stock, b.spec_value_id, b.value_id, c.spec_is_select';
                ...
                $list = Db::name('shop_cart')->field('a.*,b.aid,b.title,b.litpic,b.users_price,b.stock_count,b.prom_type,c.spec_price,c.spec_stock,c.value_id')
                    ->alias('a')
                    ->join('__ARCHIVES__ b', 'a.product_id = b.aid', 'LEFT')
                    ->join('__PRODUCT_SPEC_VALUE__ c', 'a.spec_value_id = c.spec_value_id and a.product_id = c.aid', 'LEFT')
                    ->where($cart_where)
            ...
            if (empty($PromType)) {
                ...
                if (!empty($shop_open_shipping)) {
                    // 通过省份获取运费模板中的运费价格
                    $template_money = Db::name('shop_shipping_template')->where('province_id', $AddressData['province'])->getField('template_money');
                    if (!empty($template_money)) {
                        // 省份运费价格为0时，使用统一的运费价格，固定ID为100000
                        $template_money = Db::name('shop_shipping_template')->where('province_id', '100000')->getField('template_money');
                    }
                    // 合计金额加上运费价格
            ...
            if (!empty($OrderId)) {
                ...
                foreach ($list as $key => $value) {
                    // 产品属性处理
                    $attr_value = $this->shop_service->ProductAttrProcessing($value);
                    // 产品规格处理
                    $spec_value = $this->shop_service->ProductSpecProcessing($value);
                    $Data = [
                        // 产品属性
                ...
                if (!empty($OrderId) && !empty($DetailsId)) {
                    ...
                    // 产品库存、销量处理
                    $this->shop_service->ProductStockProcessing($UpSpecValue);
    ...
    // 添加收货地址
    public function shopAddAddress()
    {
        if (isAjaxPost(false)) {
            $post = input('post.');
            ...
            $addr_id = Db::name('shop_address')->add($post);
            if (isMobile() && isWeixin() && !empty($addr_id)) {
                ...
                Db::name('shop_address')->where($AddressWhere)->update($data_new);
                $this->success('添加成功！',session($this->users_id.'_ThinkerShopOrderUrl'));
            }
        ...
        if ('list' == $types || 'order' == $types || 'order_new' == $types) {
            ...
            $addr_num = Db::name('shop_address')->where($Where)->count();
    ...
    // 更新收货地址
    public function shopEditAddress()
    {
        if (isAjaxPost(false)) {
            $post = input('post.');
            if (empty($post['consignee'])) {
            ...
            $addr_id = Db::name('shop_address')->where($AddrWhere)->update($post);
        ...
        // 根据地址ID查询相应的中文名字
        $AddrData = Db::name('shop_address')->where($AddrWhere)->find();
        if (empty($AddrData)) {
            $this->error('数据有误！');
        }
        $AddrData['country']  = '中国'; //国家
        $AddrData['Province'] = get_province_list(); // 省份
        $AddrData['City']     = Db::name('region')->where('parent_id',$AddrData['province'])->select(); // 城市
        $AddrData['District'] = Db::name('region')->where('parent_id',$AddrData['city'])->select(); // 县/区/镇
        $thinker = [
            'field' => $AddrData,
        ];
        $this->assign('thinker',$thinker);
    ...
    // 删除收货地址
    public function shopDelAddress()
    {
        if (isPost()) {
            ...
            $return = Db::name('shop_address')->where($Where)->delete();
            if ($return) {
                $this->success('删除成功！');
            }else{
                $this->error('删除失败！');
    ...
    // 更新收货地址，设置为默认地址
    public function shopSetDefaultAddress()
    {
        if (isAjaxPost(false)) {
            $post = input('post.');
            // 更新条件及数据
            ...
            if (!empty($addr_id)) {
                ...
                $data['update_time'] = getTime();
                Db::name('shop_address')->where($AddressWhere)->update($data);
                $this->success('设置成功！');
            }else{
                $this->error('数据有误！');
            }
    ...
    // 查询运费
    public function shopInquiryShipping()
    {
        if (isAjaxPost(false)) {
            $shop_open_shipping = getUsersConfigData('shop.shop_open_shipping');
            if (empty($shop_open_shipping)) {
            ...
            $province = Db::name('shop_address')->where($where)->getField('province');

            // 通过省份获取运费模板中的运费价格
            $template_money = Db::name('shop_shipping_template')->where('province_id',$province)->getField('template_money');
    ...
    // 会员提醒收货
    public function shopOrderRemind()
    {
        if (isAjaxPost(false)) {
            $post = input('post.');
            // 添加订单操作记录
    ...
    // 会员确认收货
    public function shopMemberConfirm()
    {
        if (isAjaxPost(false)) {
            $post = input('post.');
            // 更新条件
    ...
    // 获取微信收货地址
    public function shopGetWechatAddr()
    {
        if (isAjaxPost(false)) {
            // 微信配置信息
            ...
            if (empty($accesstoken)) {
                ...
                $return = $this->shop_service->GetWeChatAccessToken($appid,$appsecret);
            ...
            if (empty($jsapi_ticket)) {
                ...
                $return = $this->shop_service->GetWeChatJsapiTicket($accesstoken);
                if (empty($return['status'])) {
                    $this->error($return['prompt']);
                }else{
            ...
            // 随机字符串
            $noncestr  = $this->shop_service->GetRandomString('16');
            $noncestr  = "$noncestr";
            // 当前时间戳
            $timestamp = getTime();
            $timestamp = "$timestamp";
    ...
    // 添加微信的收货地址到数据库
    public function addWechatAddr()
    {
        if (isAjaxPost(false)) {
            $post = input('post.');
            // 省
            $province = Db::name('region')->where('name',$post['provinceName'])->getField('id');
            // 市
            $city     = Db::name('region')->where('name',$post['cityName'])->getField('id');
            // 县
            $district = Db::name('region')->where('name',$post['countryName'])->getField('id');
            ...
            $return = Db::name('shop_address')->where($where)->find();
            if (!empty($return)) {
                $this->success('获取成功！',session($this->users_id.'_ThinkerShopOrderUrl'));
            }else{
                $data = [
                    'users_id'   => $this->users_id,
                    'consignee'  => $post['userName'],
                ...
                $addr_id = Db::name('shop_address')->add($data);
                if (!empty($addr_id)) {
                    // 把对应会员下的所有地址改为非默认
                    $AddressWhere = [
                        'addr_id'  => array('NEQ',$addr_id),
    ...
    private function IsSoldOut($param = array())
    {
       if (!empty($param['aid'])) {
            if (!empty($param['spec_value_id'])) {
                ...
                $stock_count = Db::name('archives')->where($archives_where)->getField('stock_count');
