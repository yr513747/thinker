...
class WeappLogic extends Model
{
    ...
    function  __construct() {
        ...
        $this->weapp_path = root_path('weapp'); // 
        $this->data_path = root_path('data'); // 
    ...
    private function scanWeapp(){
        ...
        foreach($weapp_list as $k=>$v){
            if (!is_dir(root_path('weapp').$v) || !file_exists(root_path('weapp').$v.'/config.php')) {
                unset($weapp_list[$k]);
            }
            else
            {
                $weapp_list[$v] = include(root_path('weapp').$v.'/config.php');
                unset($weapp_list[$k]);                    
            }
        }
        return $weapp_list;
    ...
    public function checkVersion($code, $serviceVersionList = false) {
        ...
        if(!empty($serviceVersionList))
        {
            ...
            foreach ($serviceVersionList as $key => $val) {
                ...
                $introStr .= '<br/>'.str_replace(PHP_EOL, $val['intro'], '<br/>');
            }
