                ...
                switch ($val['dtype']) {
                    ...
                    case 'imgs':
                    {
                        ...
                        if (array_key_exists($val['name'], $addonRow) && !empty($addonRow[$val['name']])) {
                            $thinker_imgupload_list = @unserialize($addonRow[$val['name']]);
                            if (false === $thinker_imgupload_list) {
                                $thinker_imgupload_list = [];
                                $thinker_imgupload_data = explode(',', $addonRow[$val['name']]);
    ...
    public function dealChannelPostData($channel_id, $data = array(), $dataExt = array())
    {
        if (!empty($channel_id)) {
            ...
            foreach ($dataExt as $key => $val) {
                ...
                /*处理复选框取消选中的情况下*/
                if (preg_match('/^(.*)(_tpempty)$/', $key) && empty($val)) {
                    $key = preg_replace('/^(.*)(_tpempty)$/', '$1', $key);
                    $nowDataExt[$key] = '';
                    continue;
                }
                /*end*/

                $key = preg_replace('/^(.*)(_thinker_is_remote|_thinker_remote|_thinker_local)$/', '$1', $key);
                ...
                switch ($dtype) {
                    ...
                    case 'img':
                    {
                        $is_remote = !empty($dataExt[$key.'_thinker_is_remote']) ? $dataExt[$key.'_thinker_is_remote'] : 0;
                        if (1 == $is_remote) {
                            $val = $dataExt[$key.'_thinker_remote'];
                        } else {
                            $val = $dataExt[$key.'_thinker_local'];
                        }
                        break;
                    }
                    ...
                    {
                        $imgData = [];
                        $imgsIntroArr = !empty($dataExt[$key.'_thinker_intro']) ? $dataExt[$key.'_thinker_intro'] : [];
