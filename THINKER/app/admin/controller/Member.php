...
class Member extends Base {
    ...
    public function users_del()
    {
        ...
        if (isAjaxPost(false) && !empty($users_id)) {
            // 删除统一条件
            $Where = [
    ...
    public function level_del()
    {
        ...
        if (isAjaxPost(false) && !empty($level_id)) {
            // 查询条件
            $where = [
    ...
    {   
        ...
        $field = $this->field_type_db->field('name,title,ifoption')
            ->where([
                'name'  => ['IN', ['text','checkbox','multitext','radio','select','img']]
            ])
            ->select();
    ...
    public function attr_del()
    {
        ...
        if (isAjaxPost(false) && !empty($para_id)) {
