...
class Shop extends Base {
    ...
    public function order_del()
    {
        ...
        if (isAjaxPost(false) && !empty($order_id)) {
            // 条件数组
            $Where = [
    ...
    function GetOrderData($order_id)
    {
        ...
        foreach ($DetailsData as $key => $value) {
            ...
            $DetailsData[$key]['litpic'] = handle_subdir($DetailsData[$key]['litpic']); // 支持子目录
        }
    ...
    public function spec_template()
    {
        ...
        // 查询规格数据
        $PresetData = $this->product_spec_preset_db->order('sort_order asc, preset_id asc')->select();
        // 数组转化
