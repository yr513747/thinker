...
class Tools extends Base {
    ...
    public function export($tables = null, $id = null, $start = null,$optstep = 0)
    {
        ...
        if(isPost() && !empty($tables) && is_array($tables) && empty($optstep)){ //初始化
            $path = tpCache('global.web_sqldatapath');
            $path = !empty($path) ? $path : config('DATA_BACKUP_PATH');
            $path = trim($path, '/');
        ...
        } elseif (isPost() && is_numeric($id) && is_numeric($start) && 1 == intval($optstep)) { //备份数据
            ...
            if(false === $start){ //出错
                ...
                if(isset($tables[++$id])){
                    ...
                    if (is_dir($install_path) && file_exists($install_path)) {
                        ...
                        $dstfile = $install_path.'/thinkercms.sql';
                        if(@copy($srcfile, $dstfile)){
                            /*替换所有表的前缀为官方默认ey_，并重写安装数据包里*/
                            $thinkerDbStr = file_get_contents($dstfile);
                            $dbtables = Db::query('SHOW TABLE STATUS');
                            ...
                            $thinkerDbStr = str_replace($tableName, $eyTableName, $thinkerDbStr);
                            @file_put_contents($dstfile, $thinkerDbStr);
                            unset($thinkerDbStr);
                            /*--end*/
                        } else {
