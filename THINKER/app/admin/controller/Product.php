...
class Product extends Base
{
    ...
    public function index()
    {
        ...
        // 模型ID
        $condition['a.channel'] = array('eq', $this->channeltype);// 回收站
    ...
    public function edit()
    {
        ...
        if (is_http_url($info['litpic'])) {
            $info['is_remote'] = 1;
            $info['litpic_remote'] = handle_subdir($info['litpic']);
        } else {
            $info['is_remote'] = 0;
            $info['litpic_local'] = handle_subdir($info['litpic']);
        }
        ...
        foreach ($proimg_list as $key => $val) {
            $proimg_list[$key]['image_url'] = handle_subdir($val['image_url']); // 支持子目录
        }
