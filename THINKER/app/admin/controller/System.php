...
class System extends Base
{
    ...
    public function web()
    {
        ...
        if (isPost()) {
            ...
            if (1 == $param['web_status']) {
                    ...
                    ConfigModel::tpCache('seo',['seo_pseudo'=>1]);
            ...
            ConfigModel::tpCache($inc_type, $param);
            $this->writeGlobalParams(); // 写入全局内置参数
            $this->success('操作成功', url('System/web'));
            exit;
        }

        $config = ConfigModel::tpCache($inc_type);
        // 网站logo
        if (is_http_url($config['web_logo'])) {
            $config['web_logo_is_remote'] = 1;
            $config['web_logo_remote'] = handle_subdir($config['web_logo']);
        } else {
            $config['web_logo_is_remote'] = 0;
            $config['web_logo_local'] = handle_subdir($config['web_logo']);
        }
        ...
        /*自定义变量*/
        $thinker_row = M('config_attribute')->field('a.attr_id, a.attr_name, a.attr_var_name, a.attr_input_type, b.value, b.id, b.name')
            ->alias('a')
            ...
            ->order('a.attr_id asc')
            ->select();
        foreach ($thinker_row as $key => $val) {
            $val['value'] = handle_subdir($val['value'], 'html'); // 支持子目录
            $val['value'] = handle_subdir($val['value']); // 支持子目录
            $thinker_row[$key] = $val;
        }
        $this->assign('thinker_row',$thinker_row);
        /*--end*/

        $this->assign('config',$config);//当前配置项
        $this->assign('seo_pseudo', ConfigModel::tpCache('seo.seo_pseudo')); // URL模式
        return $this->fetch();
    }
    ...
    public function web2()
    {
        ...
        if (isPost()) {
            ...
            $web_adminlogo_old = ConfigModel::tpCache('web.web_adminlogo');
            if ($web_adminlogo != $web_adminlogo_old && !empty($web_adminlogo)) {
                ...
                if (-1 == $web_is_authortoken) {
                    $destination = '/static/admin/images/logo_ey.png';
                } else {
                    $destination = '/static/admin/images/logo.png';
                }
        ...
        if (-1 == $config['web_is_authortoken']) {
            $config['web_adminlogo'] = $this->root_dir.'/static/admin/images/logo_ey.png';
        } else {
            $config['web_adminlogo'] = $this->root_dir.'/static/admin/images/logo.png';
        }
    ...
    public function water()
    {
        ...
        if (is_http_url($config['mark_img'])) {
            ...
            $config['mark_img_remote'] = handle_subdir($config['mark_img']);
        } else {
            $config['mark_img_is_remote'] = 0;
            $config['mark_img_local'] = handle_subdir($config['mark_img']);
        }
    ...
    public function thumb()
    {
        ...
        if (isPost()) {
            ...
            $thumbConfig = ConfigModel::tpCache('thumb'); // 旧数据
    ...
    public function clear_cache()
    {
        if (isPost()) {
            ...
            /*清除旧升级备份包，保留最后一个备份文件*/
            $backupArr = glob(root_path('data').'backup/v*_www');
            for ($i=0; $i < count($backupArr) - 1; $i++) { 
                delFile($backupArr[$i], true);
            }

            $backupArr = glob(root_path('data').'backup/*');
    ...
    {
        if (empty($arr)) {
            delFile(runtime_path('html'), true);
        } else {
            
            foreach ($arr as $key => $val) {
                $fileList = glob(runtime_path('html').$val.'*');
                if (!empty($fileList)) {
    ...
    public function send_email()
    {
        ...
        if (intval($res['code']) == 1) {
               ...
               ConfigModel::tpCache('smtp',$smtp_config);
    ...
    public function send_mobile()
    {
        ...
        if (intval($res['code']) == 1) {
                ...
                ConfigModel::tpCache('sms',$sms_config);
                ...
                if ($r) {
                    ...
                    if (!empty($addData)) {
                        ...
                        foreach ($addData as $key => $val) {
                            ...
                            ConfigModel::tpCache('web', $configData);
...
<br/>
【指定宽高度的调用】<br/>
{$space}列表页/内容页：{\$thinker.field.litpic<span style="color:red">|thumb_img=###,500,500</span>}<br/>
...
$msg_code = <<<EOF
&lt;!--购物车组件start--&gt; <br/>
{sppurchase id='field' currentstyle='btn-danger'} <br/>
{$space}&lt;!-- 价格 标签开始 --&gt;  <br/>
...
{$space}{volist name="\$field.ReturnData" id='field2'} <br/>
...
{$space}{$space}{$space}{volist name="\$field2.spec_value" id='field3'} <br/>
