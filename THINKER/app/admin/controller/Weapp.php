...
class Weapp extends Base
{
    ...
    public function index()
    {
        ...
        foreach (['keywords'] as $key) {
            if (isset($get[$key]) && $get[$key] !== '') {
                if ($key == 'keywords') {
                    $condition[] = array('a.name|code','LIKE', "%{$get[$key]}%");
                } else {
                    $condition[] = array('a.'.$key,'=', $get[$key]);
                }
            }
        }
        ...
        if(! empty($this->plugins)){
            $condition[] = array('a.code','in', $this->plugins);
        }
        /*--end*/
        ...
        foreach ($list as $key => $val) {
            $config = include root_path('weapp').$val['code'].DS.'config.php';
            $config['description'] = str_replace(PHP_EOL, $config['description'], '<br/>');
            $val['config'] = $config;
            $val['version'] = !empty($config['version']) ? $config['version'] : 'v1.0';
        ...
        if (!empty($weappArr)) {
            ...
            $service_url = 'http://service.thinkercms.com'.base64_decode($tmp_str);
        ...
        $weapp_install_answer = tpSetting('weapp.weapp_install_answer');
        if (!empty($weapp_install_answer)) {
            ...
            if (!empty($firstInstallanswer) && $firstInstallanswer == $weapp_install_answer) {
                ...
                $weapp_install_ask = tpSetting('weapp.weapp_install_ask');
                $weapp_askanswer_list = config('global.weapp_askanswer_list');
    ...
    public function execute($sm = '', $sc = '', $sa = '')
    {
        ...
        $sm = $this->request->param($this->weapp_app);
        $sc = $this->request->param($this->weapp_controller);
        $sa = $this->request->param($this->weapp_action);

       

        $controllerName = !empty($sc) ? $sc : $sm;
        $actionName = !empty($sa) ? $sa : "index";
        $class_path = "\\weapp\\".$sm."\\controller\\".$controllerName;
		$weappClass = $this->app->make($class_path, [], true);
		if (method_exists($weappClass, $actionName)) {
			return $this->app->weappAction("{$sm}/{$controllerName}/{$actionName}");
        } else {
			$weappClass = is_object($weappClass) ? get_class($weappClass) : $weappClass;
            throw new \BadMethodCallException(sprintf('Method %s::%s does not exist.', $weappClass, $actionName));
        }
    }
...
/*        if (!empty($this->admin_info['parent_id']) || -1 != $this->admin_info['role_id']) {
        ...
        if (empty($this->admin_info['weapp_info']['firstInstallanswer'])) {
            $answer = input('post.answer/s');
            $installanswer = func_encrypt($answer);
            if (empty($installanswer)) {
                $this->error('请录入问题的答案内容！');
            } else {
                $weapp_install_answer = tpSetting('weapp.weapp_install_answer');
                if ($weapp_install_answer != $installanswer) {
                    $this->error('答案内容不正确！');
                }
            }
            $this->admin_info['weapp_info']['firstInstallanswer'] = $installanswer;
            session('admin_info', $this->admin_info);
        }*/
        /*--end*/
    ...
    public function pack()
    {
        if (isPost()) {
            ...
            /*压缩插件目录*/
            $zip = new \ZipArchive();//新建一个ZipArchive的对象
            $filepath = root_path('data').WEAPP_DIR_NAME;
            tp_mkdir($filepath);
