...
class Archives extends Base
{
    ...
    public function index()
    {
		...
		$arctype_list = (new ArctypeLogic())->arctypeList(0, 0, false, 0, [['is_del','=',0]], false);
        $zNodes = "[";
        foreach ($arctype_list as $key => $val) {
            ...
            if (!empty($val['weapp_code'])) {
                ...
                if (6 == $current_channel) {
                    ...
                    $typeurl = url('Archives/indexArchives', array('typeid'=>$val['id']));
                }
            }
    ...
    public function indexArchives()
    {
        ...
        foreach (['keywords','typeid','flag','is_release'] as $key) {
            if (isset($param[$key]) && $param[$key] !== '') {
                if ($key == 'keywords') {
                    $condition[] = array('a.title','LIKE', "%{$param[$key]}%");
                } else if ($key == 'typeid') {
                    $typeid = $param[$key];
                    ...
                    if (0 < intval($admin_info['role_id'])) {
                        ...
                        if(! empty($auth_role_info)){
                            if(isset($auth_role_info['only_oneself']) && 1 == $auth_role_info['only_oneself']){
								...
								$condition[] = array('a.admin_id','=', $admin_info['admin_id']);
                            }
                    ...
                    /*--end*/
                    $condition[] = array('a.typeid','IN', $typeids);
                } else if ($key == 'flag') {
                    if ('is_release' == $param[$key]) {
                        $condition[] = array('a.users_id','>', 0);
                    } else {
                        $condition[] = array('a.'.$param[$key],'=', 1);
                    }
                // } else if ($key == 'is_release') {
                //     if (0 < intval($param[$key])) {
                //         $condition[] = array('a.users_id','>', intval($param[$key]));
                //     }
                } else {
                    $condition[] = array('a.'.$key,'=', $param[$key]);
                }
            }
        }
        ...
        if (empty($typeid)) {
            ...
            $condition[] = array('a.channel','NOT IN', $channelIds);
        } else {
            // 只显示当前栏目对应模型下的文档
            $current_channel = Db::name('arctype')->where('id',$typeid)->getField('current_channel');
            $condition[] = array('a.channel','=', $current_channel);
        }

       

        /*回收站数据不显示*/
        $condition[] = array('a.is_del','=', 0);
        /*--end*/
        ...
        if ($list) {
            ...
            foreach ($list as $key => $val) {
                ...
                $row[$val['aid']]['litpic'] = handle_subdir($row[$val['aid']]['litpic']); // 支持子目录
    ...
    public function add()
    {
        ...
        if (!empty($typeid)) {
            ...
            $gourl = url('Archives/indexArchives', array('typeid'=>$typeid));
            $data['gourl'] = $gourl;
    ...
    public function release()
    {
        ...
        if (0 < $typeid) {
            ...
            $gourl = url('Archives/indexArchives', array('typeid'=>$typeid), true, true);
            $data['gourl'] = $gourl;
